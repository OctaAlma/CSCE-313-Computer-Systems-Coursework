...
void handle_process_loop (TCPRequestChannel* channel) {
	...
	// YOu will be making a thread for every channel that we create
...
int main (int argc, char* argv[]) {
	...
	vector<TCPRequestChannel*> requestChannels;
	vector<thread> processes;

	while (true) {
		int activeFD = control_channel->accept_conn();
		TCPRequestChannel* reqChan = new TCPRequestChannel(activeFD); // TODO: PUT THIS GUY INTO A THREAD with handle_process_loop

		processes.push_back(thread(handle_process_loop, reqChan));
		processes.at(processes.size()-1).detach();
	}
